    ...
    match args[2]:
        ...
        DebugShape.COORDINATE_SYSTEM:
            _draw_coordinate_system(
                args[3], args[4], args[5], args[6], args[7], args[0])
        DebugShape.GRID:
            _draw_grid(
                args[3],
                args[4],
...
func draw_stuff():
    draw_debug_cube(10, Vector3(-1, 1, -1), 0.9 *
                    Vector3(1, 1, 1), Color(10, 10, 0))
    ...
    draw_debug_cylinder(10, Vector3(2, -2, 1),
                        Vector3(3, -1, 2), 1.0, 32, true, Color(5, 0, 5))
    draw_debug_cone(10, Vector3(-4, 1, 0), Vector3(-4, 2, -1),
                    0.2, 0.5, 16, true, Color(0, 2, 0), false)

    draw_debug_sphere(10, Vector3(-2, -2, 2), 36, 18, 1.5, Color(0, 2, 2))

    draw_debug_line(10, Vector3(2, 1, 0), Vector3(-2, 0, 3),
                    0.1, Color(0.7, 0.2, 0.1))
    draw_debug_coordinate_system(
        10, Vector3.ZERO, Vector3(
            1, 0, 0), Vector3(
            0, 1, 0), 1, 10)
    draw_debug_cylinder(10, Vector3(4, 0, -3), Vector3(2, 1, -4),
                        0.5, 16, true, Color(1, 1, 1), true)
    draw_debug_cone(10, Vector3(-3, 1, 0), Vector3(-3, 2, -1),
                    0.5, 0.2, 16, true, Color(0, 2, 0), true)
    draw_debug_arrow(10, Vector3(4, 2, 1), Vector3(
        1, -3, 2), 2, Color(3, 2, 1))
    draw_debug_point(10, Vector3(2, 3, 1), 0.1, Color(0, 0, 0))
    draw_debug_coordinate_system(
        10, Vector3(-4, 0, -3), Vector3(3, 2, 1), Vector3(1, 1, 1), 0.5, 10)
...
func _draw_grid(p: Vector3, a: float, b: float, div_a: int, div_b: int,
                ...
                orthogonal: Vector3=Vector3(1, 0, 0),
                color: Color=Color(0, 0, 0)):
    if orthogonal == normal:
    ...
    var proj = orthogonal - orthogonal.dot(normal) / normal.length_squared() * normal
