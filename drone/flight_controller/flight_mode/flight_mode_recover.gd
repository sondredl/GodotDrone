...
func _get_command(_input: FlightCommand) -> FlightCommand:
    ...
    if safe_flight:
        ...
        flight_command.power = pid_speed_vertical.get_output(
            flight_state.velocity.y, dt, false)
        ...
        if ang_vel.length_squared() <= deg_to_rad(360):
            ...
            flight_command.roll = pid_roll_speed.get_output(
                -ang_vel.z, dt, false)
            pid_pitch_speed.target = 0
            flight_command.pitch = pid_pitch_speed.get_output(
                ang_vel.x, dt, false)
