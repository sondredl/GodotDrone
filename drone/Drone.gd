...
@export var rate_pitch := 667.0  # (float, 1.0, 1800.0)
@export var rate_roll := 667.0  # (float, 1.0, 1800.0)
@export var rate_yaw := 667.0  # (float, 1.0, 1800.0)
@export var expo_pitch := 0.2  # (float, 0.0, 1.0)
@export var expo_roll := 0.2  # (float, 0.0, 1.0)
@export var expo_yaw := 0.2  # (float, 0.0, 1.0)
...
# flight_controller.set_hover_rpm(hover_rpm)
    ...
    add_child(hud)
    var _discard = flight_controller.connect("armed", Callable(hud.status, "_on_armed"))
    _discard = flight_controller.connect(
        "disarmed", Callable(
            hud.status, "_on_disarmed"))
    _discard = flight_controller.connect(
        "flight_mode_changed", Callable(
            hud.status, "_on_mode_changed"))
    _discard = flight_controller.connect(
        "arm_failed", Callable(
            hud.status, "_on_arm_failed"))
    ...
    _discard = QuadSettings.connect(
        "settings_updated", Callable(
            self, "_on_quad_settings_updated"))
    _on_quad_settings_updated()

    GameSettings.load_hud_config()
    _discard = GameSettings.connect(
        "hud_config_updated", Callable(
            self, "_on_hud_config_updated"))
...
func _process(delta: float) -> void:
    ...
    if b_debug and debug_geom:
        ...
        debug_geom.draw_debug_grid(0.02,
                                   global_xform * (Vector3(0,
                                                           0,
                                                           0)),
                                   1.5,
                                   1.5,
        ...
        debug_geom.draw_debug_arrow(0.02,
                                    global_xform * (Vector3.UP),
                                    linear_velocity,
                                    linear_velocity.length() / 10)
        debug_geom.draw_debug_arrow(0.02, global_xform *
                                    (Vector3.UP), Vector3.RIGHT, linear_velocity.x /
                                    10, Color(10, 0, 0))
        debug_geom.draw_debug_arrow(0.02, global_xform *
                                    (Vector3.UP), Vector3.UP, linear_velocity.y /
                                    10, Color(0, 10, 0))
        debug_geom.draw_debug_arrow(0.02, global_xform *
                                    (Vector3.UP), Vector3.BACK, linear_velocity.z /
                                    10, Color(0, 0, 10))

        debug_geom.draw_debug_arrow(0.02, global_xform *
                                    (Vector3(0.2, 0, 0.5)), global_basis *
                                    (Vector3.RIGHT), (linear_velocity) *
                                    global_basis.x /
                                    10, Color(10, 0, 0))
        debug_geom.draw_debug_arrow(0.02,
...
func _physics_process(_delta: float) -> void:
    ...
    replay_transform = replay_transform.replace(
        "(", "").replace(")", "").replace(" ", "")
...
# prop.set_velocity((lin_vel * basis) + basis.inverse().xform(ang_vel).cross(prop_local_pos))
            ...
            var prop_drag: Vector3 = basis * (prop_forces[1])
            if b_debug and debug_geom and i == 0 and prop.name == "Propeller1":
                print(
                    "V: %5.2f, T: %5.2f, D: %5.2f" % [
                        prop.velocity.y,
                        prop_forces[0].length(),
                        prop_forces[1].length()])
                # Draw debug arrows relative to FPV camera
                debug_geom.draw_debug_arrow(0.016, xform *
                                            (Vector3(0, 1, -
            ...
            vec_torque -= prop_thrust.cross(basis * (prop_xform.origin))
...
func _on_reset() -> void:
    ...
    if Global.game_mode == Global.GameMode.RACE and Global.active_track:
        ...
        respawn_transform = launch_transform.translated(
...
func get_drag(lin_vel: Vector3, ang_vel: Vector3, orientation: Basis) -> Array:
    ...
    local_drag[0] = -local_vel.length() * local_vel * \
        projected_area * cd / 2.0 * 1.225
    local_drag[1] = -local_ang.length() * local_ang * \
        projected_area * cd / 200.0 * 1.225
...
func _on_quad_settings_updated() -> void:
    ...
    control_profile.set_rates(
        QuadSettings.rate_pitch,
        QuadSettings.rate_roll,
        QuadSettings.rate_yaw)
    control_profile.set_expo(
        QuadSettings.expo_pitch,
        QuadSettings.expo_roll,
        QuadSettings.expo_yaw)


func _on_hud_config_updated() -> void:
    hud.hud_timer = 1.0 / GameSettings.hud_config["fps"]
    hud.show_component(
        HUD.Component.CROSSHAIR,
        GameSettings.hud_config["crosshair"])
    hud.show_component(
        HUD.Component.HORIZON,
        GameSettings.hud_config["horizon"])
    ...
    hud.show_component(
        HUD.Component.ALTITUDE,
        GameSettings.hud_config["altitude"])
    hud.show_component(
        HUD.Component.HEADING,
        GameSettings.hud_config["heading"])
