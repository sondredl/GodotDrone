...
var state_armed := false: set = set_armed
...
@export var pid_roll_p := 50.0  # (float, 0.0, 1000.0)
@export var pid_roll_i := 30.0  # (float, 0.0, 1000.0)
@export var pid_roll_d := 30.0  # (float, 0.0, 1000.0)
@export var pid_pitch_p := 50.0  # (float, 0.0, 1000.0)
@export var pid_pitch_i := 30.0  # (float, 0.0, 1000.0)
@export var pid_pitch_d := 30.0  # (float, 0.0, 1000.0)
@export var pid_yaw_p := 70.0  # (float, 0.0, 1000.0)
@export var pid_yaw_i := 90.0  # (float, 0.0, 1000.0)
@export var pid_yaw_d := 40.0  # (float, 0.0, 1000.0)
...
func _ready() -> void:
    ...
    # Clamp limits for speed controllers are equal to the maximum pitch/roll
    # angle
    ...
    pid_controllers[Controller.FORWARD_SPEED].set_clamp_limits(
        -max_angle, max_angle)
    pid_controllers[Controller.LATERAL_SPEED].set_coefficients(
        50 * pid_scale_p, 50 * pid_scale_i, 1 * pid_scale_d)
    pid_controllers[Controller.LATERAL_SPEED].set_clamp_limits(
        -max_angle, max_angle)
    ...
    var _discard = connect("flight_mode_changed", Callable(get_parent(), "_on_flight_mode_changed"))
...
func integrate_loop(delta: float, drone_pos: Vector3, drone_basis: Basis) -> void:
    ...
    basis_flat = Basis(
        Vector3(
            basis.x.x, 0, basis.x.z), Vector3.UP, Vector3(
            basis.z.x, 0, basis.z.z)).orthonormalized()
...
func set_motors(motor_array: Array) -> void:
    motors = motor_array
    for motor in motors:
        var _discard = connect("armed", Callable(motor, "_on_armed"))
...
# print("%8.3f %8.3f %8.3f %8.3f" % [power, yaw, roll, pitch])
    ...
    if pwm_max - pwm_min > 1 - idle_pwm:
        ...
        for i in range(4):
            motor_pwm[i] = pwm_mid + (motor_pwm[i] - pwm_mid) * \
                (1 - idle_pwm) / (pwm_max - pwm_min)
...
func update_command() -> Array:
    ...
    if flight_mode == FlightMode.RATE:
        ...
        pid_controllers[Controller.YAW_SPEED].set_target(
            -((1 - expo_y) * y + expo_y * pow(y, 3)) * 2 * PI)
        motor_control[1] = pid_controllers[Controller.YAW_SPEED].get_output(
            ang_vel.y, dt, false)

        pid_controllers[Controller.ROLL].set_target(
            ((1 - expo_r) * r + expo_r * pow(r, 3)) * bank_limit)
        motor_control[2] = pid_controllers[Controller.ROLL].get_output(
            -angles.z, dt)

        pid_controllers[Controller.PITCH].set_target(
            ((1 - expo_p) * p + expo_p * pow(p, 3)) * bank_limit)
        motor_control[3] = pid_controllers[Controller.PITCH].get_output(
            angles.x, dt, false)
        ...
        pid_controllers[Controller.YAW_SPEED].set_target(
            -((1 - expo_y) * y + expo_y * pow(y, 3)) * 2)
        motor_control[1] = pid_controllers[Controller.YAW_SPEED].get_output(
            ang_vel.y, dt, false)

        pid_controllers[Controller.LATERAL_SPEED].set_target(
            ((1 - expo_r) * r + expo_r * pow(r, 3)) * 10.0)
        ...
        pid_controllers[Controller.FORWARD_SPEED].set_target(
            ((1 - expo_p) * p + expo_p * pow(p, 3)) * 10.0)
        ...
        target.x = target.x + ((1 - expo_r) * r +
                               expo_r * pow(r, 3)) * 5.0 * dt
        target.y = target.y + (pwr - 0.5) * 5.0 * dt
        target.z = target.z + ((1 - expo_p) * p +
                               expo_p * pow(p, 3)) * 5.0 * dt
        set_tracking_target(target)

        motor_control[0] = pid_controllers[Controller.ALTITUDE].get_output(
            pos.y, dt, false)
        ...
        motor_control[1] = pid_controllers[Controller.YAW].get_output(
            measurement, dt, false)
...
# pid_controllers[Controller.LATERAL_SPEED].set_target(lateral_speed_target)
        ...
        motor_control[3] = pid_controllers[Controller.LAUNCH].get_output(
            angles.x, dt, false)
