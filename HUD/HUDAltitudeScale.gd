...
func update_altitude(a: float, dt: float) -> void:
    ...
    if altitude < 0:
        alt_sign = "-"
    altitude_label.text = "%s%04d.%d" % [
        alt_sign, abs(a), (abs(a) - floor(abs(a))) * 10]
    ...
    if vertical_speed < 0:
        vs_sign = "-"
    vertical_speed_label.text = "%s%02d.%d VS" % [vs_sign, abs(
        vertical_speed), (abs(vertical_speed) - floor(abs(vertical_speed))) * 10]
