...
func update_altitude(new_altitude: float, dt: float) -> void:
    ...
    if altitude < 0:
        alt_sign = "-"
    altitude_label.text = "%s%04d.%d" % [alt_sign, absf(
        new_altitude), (absf(new_altitude) - floori(absf(new_altitude))) * 10]
    ...
    vertical_speed_label.text = "%s%02d.%d VS" % [vs_sign, absf(
        vertical_speed), (absf(vertical_speed) - floori(absf(vertical_speed))) * 10]
...
func update_radio_altitude(new_altitude: float) -> void:
    if new_altitude > 10 or new_altitude < 0:
        ...
        radioaltitude_label.text = "R%02d.%d" % [
            new_altitude, (new_altitude - floori(new_altitude)) * 10]
