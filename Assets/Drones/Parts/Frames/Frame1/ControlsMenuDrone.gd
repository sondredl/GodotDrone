...
func _ready() -> void:
    ...
    for i in range(4):
        if i == 1 or i == 3:
            prop = "CCW"
        get_node(
            "Motor%d/Propeller%d/%s" %
            [
                i +
                1,
                i +
                1,
                prop]) .mesh.surface_get_material(0).set_shader_parameter(
            "propeller_color",
            color)


func _process(delta: float) -> void:
    transform = Transform3D.IDENTITY.rotated(Vector3.UP, -yaw * ANGLE) .rotated(
        Vector3.RIGHT, pitch * ANGLE).rotated(Vector3.FORWARD, roll * ANGLE)
