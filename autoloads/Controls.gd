...
func update_active_device(device: int) -> int:
    ...
    if err == OK or err == ERR_FILE_NOT_FOUND:
        ...
        if err != OK:
            Global.log_error(
                err, "Error while updating active device in config file.")
    else:
        Global.log_error(
            err, "Error while updating active device in config file.")
    return err
...
func load_input_map(update_controller: bool=false) -> String:
    ...
    if err == OK:
        ...
        active_controller_guid = config.get_value(
            "controls", "active_controller_guid")
        ...
        if update_controller:
            ...
            if config.has_section_key("controls", "default_controller"):
                default_controller_guid = config.get_value(
                    "controls", "default_controller")
                ...
                if action.begins_with("throttle") or action.begins_with("yaw") \
                    ...
                    if binding_type == ControllerAction.Type.BUTTON:
                        if action == current_action + "_button":
                            action_list[action_idx].button = config.get_value(
                                section, action)
                        ...
                        if action == current_action + "_axis":
                            action_list[action_idx].axis = config.get_value(
                                section, action)
