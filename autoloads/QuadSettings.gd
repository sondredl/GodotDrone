...
func load_quad_settings() -> void:
    ...
    if err == OK:
        ...
        if config.has_section_key("quad", "dry_weight"):
            dry_weight = clampf(
                config.get_value(
                    "quad", "dry_weight"), 0.1, 1.0)
        if config.has_section_key("quad", "battery_weight"):
            battery_weight = clampf(
                config.get_value(
                    "quad", "battery_weight"), 0.1, 0.5)
        ...
        if config.has_section_key("rates", "pitch_expo"):
            control_profile.pitch_expo = clampf(
                config.get_value("rates", "pitch_expo"), 0.0, 1.0)
        if config.has_section_key("rates", "roll_expo"):
            control_profile.roll_expo = clampf(
                config.get_value("rates", "roll_expo"), 0.0, 1.0)
        if config.has_section_key("rates", "yaw_expo"):
            control_profile.yaw_expo = clampf(
                config.get_value("rates", "yaw_expo"), 0.0, 1.0)
        return
