...
class DebugGrid extends DebugShape:
    ...
    func _draw_debug_shape() -> void:
        ...
        if normal_direction == Vector3.DOWN:
            ...
            if normal_direction.cross(
                    Vector3.UP).normalized() == -normal_direction.normalized():
        ...
        if rotated_right_vector.cross(
                projection).normalized() != normal_direction.normalized():
            tangent_angle = -tangent_angle

        for i in range(0, length_divs + 1):
            var lx := length * (i as float / length_divs - 0.5)
            add_line_primitive([Vector3(lx,
                                        0,
                                        -width / 2.0).rotated(normal_rotation,
                                                              normal_angle).rotated(tangent_rotation,
                                                                                    tangent_angle) + position,
                                Vector3(lx,
                                        0,
                                        width / 2.0).rotated(normal_rotation,
                                                             normal_angle).rotated(tangent_rotation,
        ...
        for j in range(0, width_divs + 1):
            ...
            add_line_primitive([Vector3(-length / 2.0,
                                        0,
                                        lz).rotated(normal_rotation,
                                                    normal_angle).rotated(tangent_rotation,
                                                                          tangent_angle) + position,
                                Vector3(length / 2.0,
                                        0,
