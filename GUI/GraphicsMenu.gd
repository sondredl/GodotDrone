...
func _ready() -> void:
    ...
    var _discard = window_mode.connect("item_selected", Callable(self, "_on_window_mode_changed"))
    ...
    _discard = game_msaa.connect(
        "item_selected", Callable(
            self, "_on_msaa_changed"))
    ...
    _discard = shadows.connect(
        "item_selected", Callable(
            self, "_on_shadows_changed"))
    ...
    _discard = fisheye_mode.connect(
        "item_selected", Callable(
            self, "_on_fisheye_mode_changed"))
    ...
    _discard = fisheye_resolution.connect(
        "item_selected", Callable(
            self, "_on_fisheye_resolution_changed"))
    ...
    _discard = fisheye_msaa.connect(
        "item_selected", Callable(
            self, "_on_fisheye_msaa_changed"))
    fisheye_msaa.select(Graphics.graphics_settings["fisheye_msaa"])

    _discard = $PanelContainer / VBoxContainer / HBoxContainer / ButtonBack.connect("pressed", Callable(self, "_on_back_pressed"))
