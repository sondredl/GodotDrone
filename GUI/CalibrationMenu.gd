...
func _ready() -> void:
    var _discard = $PanelContainer / VBoxContainer / ButtonCancel.connect("pressed", Callable(self, "_on_cancel_pressed"))
    _discard = connect(
        "calibration_done", Callable(
