...
func _ready() -> void:
    ...
    label.append_text("\n\n")
    label.append_text(
        "You need a gamepad or radio transmitter to play, or really any device with 4 ")
    ...
    label.append_text("\n\n")
    label.append_text(
        "All stabilized modes hold altitude as well. Stabilized modes trigger recovery ")
    ...
    label.append_text(
        "should be flicked/pressed. In the controls menu, \"Arm (hold)\" is supposed to ")
    label.append_text(
        "be used with a switch, and \"Arm (toggle)\" is supposed to be used with a button.")
    label.append_text("\n\n")
    label.append_text(
        "Assigning the same axis to multiple actions is possible for potentiometers ")
    label.append_text(
        "and 3-position switches, as you can define the active range of every action ")
