...
@onready var skeleton := $RadioTransmitter / Armature / Skeleton3D
...
func _input(event) -> void:
    if event is InputEventJoypadMotion and accept_input:
        ...
        if event.is_action("throttle_up") or event.is_action("throttle_down"):
            ...
            axis = Vector3.RIGHT
            value = event.get_action_strength(
                "throttle_down") - event.get_action_strength("throttle_up")
        ...
        if stick > -1:
            ...
            skeleton.set_bone_pose(
                stick, Transform3D.IDENTITY.rotated(
                    axis, angle))
...
func set_left_stick_horizontal(value: float) -> void:
    var angle := get_stick_angle(value)
    skeleton.set_bone_pose(
        Stick.LEFT_HORIZONTAL,
        Transform3D.IDENTITY.rotated(
            Vector3.FORWARD,
            angle))
    ...
    match calibration_step:
        0:
            ...
            tween.interpolate_method(self, "set_right_stick_vertical", 0, -1,
                                     0.5, Tween.TRANS_CUBIC, Tween.EASE_IN_OUT)
            var _discard = tween.connect("tween_all_completed", Callable(self, "loop_stick_animation"))
        1:
...
func _on_calibration_step_changed(step: int) -> void:
    if step == 1:
        tween.disconnect(
            "tween_all_completed", Callable(
                self, "loop_stick_animation"))
    play_animation(step)
