...
func _ready() -> void:
    var _discard = connect("controller_detected", Callable(self, "_on_controller_autodetected"))
    _discard = Input.connect(
        "joy_connection_changed", Callable(
            self, "_on_joypad_connection_changed"))

    _discard = $MenuPanel / MenuVBox / ButtonCalibrate.connect("pressed", Callable(self, "_on_calibrate_pressed"))
    _discard = $MenuPanel / MenuVBox / ButtonBack.connect("pressed", Callable(self, "_on_back_pressed"))

    _discard = controller_list.connect(
        "pressed", Callable(
            self, "_on_controller_list_pressed"))
    _discard = controller_list.get_popup().connect(
        "id_pressed", Callable(self, "_on_controller_selected"))
    _discard = controller_list.get_popup().connect(
        "modal_closed", Callable(
            self, "_on_controller_select_aborted"))
    controller_list.get_popup().add_theme_font_override(
        "font", load("res://GUI/MenuFont.tres"))
    controller_list.clip_text = true

    _discard = controller_checkbutton.connect(
        "toggled", Callable(self, "_on_checkbutton_toggled"))

    # TODO: only allow calibration for active controller OR update controller
    # list
    ...
    if !active_controller_found:
        ...
        if default_controller >= 0:
            for joypad in connected_joypads:
                if Input.get_joy_guid(
                        joypad) == Controls.default_controller_guid:
    ...
    controller_list.get_popup().emit_signal(
        "id_pressed", connected_joypads.find(active_device))
    ...
    for action in Controls.action_list:
        ...
        _discard = binding.connect("clicked", Callable(
            self, "_on_binding_clicked").bind(binding))
        _discard = binding.connect(
            "binding_updated", Callable(
                self, "_on_binding_updated"))
    ...
    if event is InputEventJoypadMotion or event is InputEventJoypadButton:
        if auto_detect_controller:
            if event is InputEventJoypadMotion and abs(
                    event.axis_value) > 0.8 or event is InputEventJoypadButton:
...
func _on_controller_autodetected(device: int) -> void:
    ...
    controller_list.get_popup().emit_signal(
        "id_pressed", connected_joypads.find(device))
...
func _on_checkbutton_toggled(pressed: bool) -> void:
    ...
    if err != OK:
        Global.log_error(
            err, "Error while saving default controller settings.")
