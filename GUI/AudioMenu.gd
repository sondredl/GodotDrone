...
func _ready() -> void:
    var _discard = master_volume.connect("value_changed", Callable(self, "_on_master_volume_changed"))
    master_volume.value = volume_to_slider(
        Audio.audio_settings["master_volume"])
