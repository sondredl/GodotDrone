...
func _ready() -> void:
    ...
    _discard = Input.joy_connection_changed.connect(
        _on_joypad_connection_changed)
    ...
    _discard = controller_list.get_popup().popup_hide.connect(
        _on_controller_select_aborted)
# controller_list.get_popup().add_theme_font_override("font", load("res://GUI/MenuFont.tres"))
    ...
    controller_list.get_popup().id_pressed.emit(
        connected_joypads.find(active_device))
    ...
    # TODO: add animated radio sticks display with customizable mode1, mode2,
    # etc.
...
func _unhandled_input(event: InputEvent) -> void:
    if event is InputEventJoypadMotion or event is InputEventJoypadButton:
        if auto_detect_controller:
            if event is InputEventJoypadMotion and absf(
...
func _on_calibrate_pressed() -> void:
    if packed_calibration_menu.can_instantiate():
        ...
        calibration_menu.calibration_step_changed.emit(
        ...
        for action in Controls.action_list:
            ...
            if action.bound:
                ...
                if action.type == ControllerAction.Type.BUTTON:
                    config.set_value(
                        section, action_name + "_button", action.button)
                elif action.type == ControllerAction.Type.AXIS:
                    config.set_value(
                        section, action_name + "_axis", action.axis)
                    config.set_value(
