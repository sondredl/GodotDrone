...
func _ready() -> void:
    ...
    axis_range.texture_progress = load(
        "res://Assets/GUI/ControlAxes_Transparent.png")
    ...
    var axis_size := axis_monitor.custom_minimum_size
    button_low.custom_minimum_size = Vector2(20, axis_size.y + 20)
    var button_low_pos := (axis_size - button_low.custom_minimum_size + bound_low * Vector2(axis_size.x, 0)) / 2
    button_low.position = button_low_pos
    button_high.custom_minimum_size = Vector2(20, axis_size.y + 20)
    var button_high_pos := (axis_size - button_high.custom_minimum_size + bound_high * Vector2(axis_size.x, 0)) / 2
    ...
    var _discard = button_low.connect("button_down", Callable(self, "_on_button_pressed").bind(1))
    _discard = button_high.connect(
        "button_down", Callable(
            self, "_on_button_pressed").bind(2))
    _discard = button_low.connect(
        "button_up", Callable(
...
func update_bounds() -> void:
    bound_low = (button_low.position.x + button_low.size.x / 2) / \
        axis_monitor.size.x * 2 - 1
    bound_high = (button_high.position.x + button_high.size.x /
                  2) / axis_monitor.size.x * 2 - 1
...
func set_bounds(low: float, high: float) -> void:
    button_low.position.x = (
        low + 1) * axis_monitor.size.x / 2 - button_low.size.x / 2
    button_high.position.x = (
        high + 1) * axis_monitor.size.x / 2 - button_high.size.x / 2
    update_bounds()
