...
func _ready() -> void:
    ...
    var _discard = angle_slider.connect("value_changed", Callable(self, "_on_angle_changed"))
    angle_slider.value = QuadSettings.angle

    _discard = dry_weight_slider.connect(
        "value_changed", Callable(
            self, "_on_dry_weight_changed"))
    _discard = battery_weight_slider.connect(
        "value_changed", Callable(
            self, "_on_battery_weight_changed"))
    dry_weight_slider.value = QuadSettings.dry_weight * 1000
    battery_weight_slider.value = QuadSettings.battery_weight * 1000

    _discard = rate_pitch_slider.connect(
        "value_changed", Callable(
            self, "_on_rate_changed").bind(rate_pitch_slider))
    _discard = rate_roll_slider.connect(
        "value_changed", Callable(
            self, "_on_rate_changed").bind(rate_roll_slider))
    _discard = rate_yaw_slider.connect(
        "value_changed", Callable(
            self, "_on_rate_changed").bind(rate_yaw_slider))
    _discard = expo_pitch_slider.connect(
        "value_changed", Callable(
            self, "_on_expo_changed").bind(expo_pitch_slider))
    _discard = expo_roll_slider.connect(
        "value_changed", Callable(
            self, "_on_expo_changed").bind(expo_roll_slider))
    _discard = expo_yaw_slider.connect(
        "value_changed", Callable(
            self, "_on_expo_changed").bind(expo_yaw_slider))
    ...
    _discard = $PanelContainer / VBoxContainer / ButtonsHBox / ButtonResetQuad.connect("pressed", Callable(self, "_on_reset_quad_pressed"))
    _discard = $PanelContainer / VBoxContainer / ButtonsHBox / ButtonResetRates.connect("pressed", Callable(self, "_on_reset_rates_pressed"))
    _discard = $PanelContainer / VBoxContainer / ButtonsHBox / ButtonBack.connect("pressed", Callable(self, "_on_back_pressed"))
...
func update_graph() -> void:
    rate_graph.update_rates(
        Vector3(
            rate_pitch_slider.value,
            rate_roll_slider.value,
            rate_yaw_slider.value),
        Vector3(
            expo_pitch_slider.value,
            expo_roll_slider.value,
            expo_yaw_slider.value))
